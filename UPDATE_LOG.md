...
## 7. 🪜 L2 深度階梯與主力掛單 UI 重構 (Professional DOM Ladder Upgrade)

依據專業期貨與加密交易所（Trading Technologies MD Trader、Bookmap、Binance Futures、Bybit）規格，重構原先粗重的 5 個靜態卡片，升級為高密度多檔位主力深度階梯：
* **多價位連續深度階梯**：
  * 由原本僅 5 個價位卡片擴充為 **16~28 檔連續雙向盤口**（賣盤 Asks 上方排列、買盤 Bids 下方排列、中央 Spot 現貨成交帶）。
  * 支援 **聚合步長（Tick Size Aggregator）**：`0.01`（精細跳動）、`0.05`（標準）、`0.10`（主力波段）、`0.50`（宏觀跨度）。
  * 支援 **盤口視角切換**：`全部`、`僅看賣盤`、`僅看買盤`。
* **主力巨鯨大單牆（Whale Wall）自動識別**：
  * 自動捕捉掛單量顯著突出的流動性牆（Whale Walls），以金色流動性高亮與 `🐳 巨鯨掛單牆` 標籤呈現。
  * 機構關鍵防線（R2 突破防線、R1 前高阻力、S1 均線防線、S2 密集底）精確吸附映射在實際價位行上。
* **多空掛單失衡比（OBI - Order Book Imbalance）**：
  * 即時統計買方與賣方掛單深度百分比（如 🟢 58% vs 42% 🔴），視覺化判斷多空優勢。
* **單擊連動風控引擎（One-Click Execution）**：
  * 盤口任意價位行可一鍵點選 `[TP]`（設為止盈位）、`[SL]`（設為止損位）或 `[入]`（設為進場價）。
  * 階梯中直接釘選用戶自訂的 `🎯 TP 止盈`、`🛡️ SL 止損`、`⚡ ENTRY` 標記徽章，實時觀察個人委託單在主力流動性中的相對位置。
